...
static SDL_Joystick *raw_joystick = NULL; /* Xbox: fallback when GameController mapping unavailable */
...
#ifdef _XBOX
		|| 1  // On Xbox there are no environment variables; always use SDL renderer
#endif
	) {
...
#ifdef _XBOX
	inputdevices = 1; // Xbox has no mouse; controller added below if present
#else
...
#ifdef _XBOX
			xbox_log("Xbox: SDL_NumJoysticks=%d\n", numjoysticks);
#endif
			if (numjoysticks < 1) {
...
#ifdef _XBOX
					xbox_log("Xbox: GameController opened: %s\n", SDL_GameControllerName(controller));
#endif
					inputdevices |= 4;
...
#ifdef _XBOX
					xbox_log("Xbox: no GameController, trying raw joystick\n");
					if (numjoysticks > 0) {
						raw_joystick = SDL_JoystickOpen(0);
						if (raw_joystick) {
							xbox_log("Xbox: raw joystick opened: %s axes=%d buttons=%d hats=%d\n",
								SDL_JoystickName(raw_joystick),
							...
							xbox_log("Xbox: SDL_JoystickOpen(0) failed: %s\n", SDL_GetError());
						}
					}
#endif
				}
...
{
#ifdef _XBOX
	(void)a; // No mouse on Xbox; SDL_SetRelativeMouseMode crashes (no WarpMouse).
	return;
#endif
...
static void enumdisplays(void)
{
	...
	displaycnt = n;
#ifdef _XBOX
	/* Xbox: SDL display enumeration may fail entirely. Inject a fallback 1280x720 display. */
	if (displaycnt == 0) {
		buildputs("Xbox: no displays enumerated, injecting fallback 1280x720 display\n");
		if (!displays)
			displays = (struct displayinfo *)calloc(1, sizeof(struct displayinfo));
		if (displays) {
			displaycnt = 1;
			displays[0].index = 0;
			strncpy(displays[0].name, "Xbox Display", sizeof(displays[0].name)-1);
			displays[0].mode.w = 1280;
			displays[0].mode.h = 720;
			displays[0].mode.format = SDL_PIXELFORMAT_ARGB8888;
			displays[0].mode.refresh_rate = 60;
			displays[0].bounds.x = 0;
			displays[0].bounds.y = 0;
			displays[0].bounds.w = 1280;
			displays[0].bounds.h = 720;
			displays[0].usablebounds = displays[0].bounds;
		}
	}
#endif
}
...
int setvideomode(int xdim, int ydim, int bitspp, int fullsc)
{
	...
	{
		static struct displayinfo xbox_di;
		if (!displays || displaycnt == 0) {
			memset(&xbox_di, 0, sizeof(xbox_di));
			xbox_di.bounds.w = xdim;
			xbox_di.bounds.h = ydim;
			displays = &xbox_di;
			displaycnt = 1;
		}
	}
	...
	do {
		...
		winw = xdim; winh = ydim;
#ifdef _XBOX
		// On Xbox, create window at the display output resolution (e.g. 640x480)
		// so the 320x240 game texture gets scaled up to fill the screen.
		{
			VIDEO_MODE vm = XVideoGetMode();
...
#ifdef _XBOX
		xbox_log("Xbox: SDL_CreateWindow(%d,%d,%d,%d,0x%x) -> %s\n",
			winx, winy, winw, winh, flags, sdl_window ? "OK" : SDL_GetError());
#endif
		if (!sdl_window) {
		...
		if (glunavailable
			...
			if (usesdlrenderer) {
...
#ifdef _XBOX
					0  /* No PRESENTVSYNC on Xbox — nxdk SDL may block */
#else
					SDL_RENDERER_PRESENTVSYNC
#endif
				);
#ifdef _XBOX
				xbox_log("Xbox: SDL_CreateRenderer -> %s\n", sdl_renderer ? "OK" : SDL_GetError());
#endif
				if (!sdl_renderer) {
				...
				sdl_texture = SDL_CreateTexture(sdl_renderer,
					...
					SDL_TEXTUREACCESS_STREAMING, xdim, ydim);
				#ifdef _XBOX
				xbox_log("Xbox: SDL_CreateTexture -> %s\n", sdl_texture ? "OK" : SDL_GetError());
#endif
				if (!sdl_texture) {
...
#ifdef _XBOX
	{
		static int sf_once = 0;
		if (!sf_once) { sf_once = 1; xbox_log("Xbox: showframe called tex=%d ren=%d frame=%d\n", sdl_texture!=0, sdl_renderer!=0, frame!=0); }
	}
#endif
#if USE_OPENGL
	if (!glunavailable) {
		...
		if (bpp != 8) {
			if (xbox_pbkit_initialized) {
				...
				if (sf_poly_count < 5) xbox_log("Xbox: showframe #%d done\n", sf_poly_count);
				sf_poly_count++;
...
#ifdef _XBOX
			xbox_log("Xbox: SDL_LockTexture FAILED\n");
#endif
...
#ifdef _XBOX
		{
			static int ro_once = 0;
			if (!ro_once) { ro_once = 1; xbox_log("Xbox: rendout=%dx%d xres=%d yres=%d\n", rendx, rendy, xres, yres); }
		}
#endif
	} else {
...
#ifdef _XBOX
		{
			static int dr_once = 0;
			if (!dr_once) { dr_once = 1; xbox_log("Xbox: destrect=%d,%d %dx%d\n", destrect.x, destrect.y, destrect.w, destrect.h); }
		}
#endif
		...
		switch (ev.type) {
...
#ifdef _XBOX
			/* Xbox OG controller arrives as a raw joystick (no GameControllerDB entry).
			 * Map SDL_JOY* events to joyb/joyaxis arrays used by SDL_CONTROLLER events. */
			...
			case SDL_JOYBUTTONUP:
				if (appactive) {
					...
					if (btn < (int)(sizeof(xbox_btn_map)/sizeof(xbox_btn_map[0]))) {
						int mapped = xbox_btn_map[btn];
			...
			case SDL_JOYHATMOTION:
				if (appactive) {
					...
					if (ev.jhat.value & SDL_HAT_UP)    joyb |= 1 << SDL_CONTROLLER_BUTTON_DPAD_UP;
					if (ev.jhat.value & SDL_HAT_DOWN)  joyb |= 1 << SDL_CONTROLLER_BUTTON_DPAD_DOWN;
					if (ev.jhat.value & SDL_HAT_LEFT)  joyb |= 1 << SDL_CONTROLLER_BUTTON_DPAD_LEFT;
					if (ev.jhat.value & SDL_HAT_RIGHT) joyb |= 1 << SDL_CONTROLLER_BUTTON_DPAD_RIGHT;
				}
				break;
			...
			case SDL_JOYAXISMOTION:
				if (appactive) {
					...
					if (axis < (int)(sizeof(xbox_axis_map)/sizeof(xbox_axis_map[0])))
						joyaxis[xbox_axis_map[axis]] = ev.jaxis.value;
				}
				break;
#endif
