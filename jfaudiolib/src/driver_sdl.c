...
#ifdef _XBOX
static int fillDataCallCount = 0;
#endif
...
static void fillData(void * userdata, Uint8 * ptr, int remaining)
{
    ...
    while (remaining > 0) {
        if (MixBufferUsed == MixBufferSize) {
            MixCallBack();
#ifdef _XBOX
            /* Log mixer output periodically to diagnose music playback */
            if (fillDataCallCount <= 10 || (fillDataCallCount % 500 == 0)) {
                const short *s = (const short *)(MixBuffer + MixBufferCurrent * MixBufferSize);
                xbox_log("SDLDrv: MixCB#%d cur=%d sz=%d s[0]=%d s[1]=%d s[2]=%d s[3]=%d\n",
                    fillDataCallCount, MixBufferCurrent, MixBufferSize, s[0], s[1], s[2], s[3]);
            }
#endif
...
int SDLDrv_PCM_Init(int * mixrate, int * numchannels, int * samplebits, void * initdata)
{
...
#ifdef _XBOX
    xbox_log("SDLDrv: PCM_Init inited=0x%x rate=%d ch=%d bits=%d\n",
        (unsigned)inited, *mixrate, *numchannels, *samplebits);
#endif
    ...
    if (err < 0) {
#ifdef _XBOX
        xbox_log("SDLDrv: SDL_Init(AUDIO) FAILED: %s\n", SDL_GetError());
#endif
...
#ifdef _XBOX
    xbox_log("SDLDrv: SDL_OpenAudio spec=%dHz/%dch/%dbit -> err=%d actual=%dHz/%dch\n",
        spec.freq, spec.channels, *samplebits, err, actual.freq, actual.channels);
#endif
    if (err < 0) {
#ifdef _XBOX
        xbox_log("SDLDrv: SDL_OpenAudio FAILED: %s\n", SDL_GetError());
#endif
    ...
    if (actual.freq == 0 || actual.channels == 0) {
        ...
        SDL_CloseAudio();
#ifdef _XBOX
        xbox_log("SDLDrv: SDL_OpenAudio returned zero freq/channels\n");
#endif
        ErrorCode = SDLErr_OpenAudio;
        return SDLErr_Error;
    }
    ...
    *mixrate = actual.freq;
#ifdef _XBOX
    xbox_log("SDLDrv: actual.format=0x%x S16SYS=0x%x S16LSB=0x%x\n",
        (unsigned)actual.format, (unsigned)AUDIO_S16SYS, (unsigned)AUDIO_S16LSB);
#endif
...
#ifdef _XBOX
        xbox_log("SDLDrv: format mismatch: actual=%s (0x%x)\n", format, (unsigned)actual.format);
#endif
    ...
    if (err) {
        SDL_CloseAudio();
#ifdef _XBOX
        xbox_log("SDLDrv: PCM_Init FAILED (format/channel mismatch)\n");
#endif
        return SDLErr_Error;
    } else {
        Initialised = 1;
#ifdef _XBOX
        xbox_log("SDLDrv: PCM_Init OK mixrate=%d bits=%d ch=%d\n",
            *mixrate, *samplebits, *numchannels);
#endif
        return SDLErr_Ok;
    }
}
...
{
#ifdef _XBOX
    xbox_log("SDLDrv: BeginPlayback buf=%p size=%d divs=%d init=%d\n",
        (void*)BufferStart, BufferSize, NumDivisions, Initialised);
#endif
    if (!Initialised) {
    ...
    SDL_PauseAudio(0);
#ifdef _XBOX
    xbox_log("SDLDrv: SDL_PauseAudio(0) called, Playing=1\n");
#endif
