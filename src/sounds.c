...
#ifdef _XBOX
    xbox_log("playmusic: fn='%s' MusicToggle=%d MusicDevice=%d\n", fn, MusicToggle, MusicDevice);
#endif
...
#ifdef _XBOX
                xbox_log("playmusic: try[%d] '%s'\n", alt, testfn);
#endif
                fp = kopen4load(testfn, 0);
#ifdef _XBOX
                xbox_log("playmusic: try[%d] result=%d\n", alt, fp);
#endif
            }
            free(testfn);
        }
...
#ifdef _XBOX
    xbox_log("playmusic: final fp=%d\n", fp);

    // If kopen4load failed, try direct absolute paths on Xbox.
    // The searchpath system relies on Bstat() which can't stat Xbox drive roots,
    // so kopen4load may fail even when files exist on disk.
    if (fp < 0 && ext && !strcasecmp(ext, ".mid")) {
        char basefn[64];
        int baselen = (int)(ext - fn);
        if (baselen > (int)sizeof(basefn)-1) baselen = (int)sizeof(basefn)-1;
        memcpy(basefn, fn, baselen);
        basefn[baselen] = 0;

        static const char * const xbox_music_dirs[] = {
            "D:\\music\\",
            "D:\\",
        ...
        char abspath[128];
        for (int d = 0; xbox_music_dirs[d] && fp < 0; d++) {
            // Try original case .ogg
            snprintf(abspath, sizeof(abspath), "%s%s.ogg", xbox_music_dirs[d], basefn);
            xbox_log("playmusic: xbox direct try '%s'\n", abspath);
            int dfd = _open(abspath, 0x8000 | 0x0000); // _O_BINARY | _O_RDONLY
            if (dfd >= 0) {
                long len = _lseek(dfd, 0, 2); // SEEK_END
                _lseek(dfd, 0, 0); // SEEK_SET
                if (len > 0) {
                    MusicLen = len;
                    MusicPtr = (char *)malloc(MusicLen);
                    if (MusicPtr && _read(dfd, MusicPtr, MusicLen) == MusicLen) {
                        _close(dfd);
                        xbox_log("playmusic: xbox direct loaded %d bytes from '%s'\n", MusicLen, abspath);
                        goto xbox_play;
                    }
                    if (MusicPtr) { free(MusicPtr); MusicPtr = 0; }
                }
                _close(dfd);
            }
            // Try lowercase .ogg
            ...
            snprintf(abspath, sizeof(abspath), "%s%s.ogg", xbox_music_dirs[d], basefn);
            xbox_log("playmusic: xbox direct try '%s'\n", abspath);
            dfd = _open(abspath, 0x8000 | 0x0000);
            if (dfd >= 0) {
                long len = _lseek(dfd, 0, 2);
                _lseek(dfd, 0, 0);
                if (len > 0) {
                    MusicLen = len;
                    MusicPtr = (char *)malloc(MusicLen);
            ...
            // Restore original case for next dir
            memcpy(basefn, fn, baselen);
            basefn[baselen] = 0;
        }
    }
...
#ifdef _XBOX
       xbox_log("playmusic: FX_PlayLoopedAuto returned %d\n", MusicVoice);
#endif
       MusicIsWaveform = 1;
    }
...
void sound(short num)
{
...
#ifdef _XBOX
    {
        static int snd_log_count = 0;
        if (++snd_log_count <= 5)
            xbox_log("sound(%d) FXDevice=%d SndTog=%d VoiceAvail=%d ptr=%p\n",
                (int)num, (int)FXDevice, (int)SoundToggle,
                FX_VoiceAvailable(soundpr[num]), (void*)Sound[num].ptr);
    }
#endif
    ...
    if(Sound[num].ptr == 0) { if( loadsound(num) == 0 ) {
#ifdef _XBOX
        xbox_log("sound(%d): loadsound FAILED\n", (int)num);
#endif
        return;
    } }
    else
