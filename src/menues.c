...
int loadpheader(char spot,struct savehead *saveh)
{
...
#ifdef _XBOX
    buildprintf("loadpheader: trying '%s'\n", fn);
#endif
    if ((fil = kopen4load(fn,0)) == -1) {
#ifdef _XBOX
        buildprintf("loadpheader: '%s' not found\n", fn);
#endif
        return(-1);
    }
#ifdef _XBOX
    buildprintf("loadpheader: '%s' opened OK (fil=%d)\n", fn, fil);
#endif
    ...
    if(bv != BYTEVERSION) {
#ifdef _XBOX
        buildprintf("loadpheader: BYTEVERSION mismatch (got %d, expected %d)\n", bv, BYTEVERSION);
#endif
...
int loadplayer(signed char spot)
{
...
#ifdef _XBOX
    buildprintf("loadplayer: trying '%s'\n", fnptr);
#endif
    if ((fil = kopen4load(fnptr,0)) == -1) {
#ifdef _XBOX
        buildprintf("loadplayer: '%s' not found\n", fnptr);
#endif
        return(-1);
    }
#ifdef _XBOX
    buildprintf("loadplayer: '%s' opened OK\n", fnptr);
#endif
    ...
    {
#ifdef _XBOX
        buildprintf("loadplayer: BYTEVERSION mismatch (got %d, expected %d)\n", bv, BYTEVERSION);
#endif
        FTA(114,&ps[myconnectindex]);
        kclose(fil);
...
#ifdef _XBOX
        buildprintf("saveplayer: fopen FAILED for '%s'\n", fnptr);
#endif
        return(-1);
    }
#ifdef _XBOX
    buildprintf("saveplayer: fopen OK\n");
#endif
        ...
        case 1009:
...
#ifdef _XBOX
            {
                static int loadconfirm_logonce = 0;
                if (!loadconfirm_logonce) {
                    buildprintf("LOAD confirm: menu=%d btn0=%d btn1=%d\n",
                        current_menu, uinfo.button0, uinfo.button1);
                    loadconfirm_logonce = 1;
                }
                if (uinfo.button0 || uinfo.button1) {
                    buildprintf("LOAD confirm: btn0=%d btn1=%d\n", uinfo.button0, uinfo.button1);
                    loadconfirm_logonce = 0;
                }
            }
#endif
            ...
            if (uinfo.button0 || KB_KeyPressed(sc_Y))
            {
...
#ifdef _XBOX
                buildprintf("LOAD confirm: YES, slot=%d\n", lastsavedpos);
#endif
                KB_FlushKeyboardQueue();
                KB_ClearKeysDown();
                if(ud.multimode < 2 && ud.recstat != 2)
                ...
                else
                {
                    c = loadplayer(lastsavedpos);
#ifdef _XBOX
                    buildprintf("LOAD confirm: loadplayer returned %d\n", c);
#endif
                    if(c == 0)
                        ps[myconnectindex].gm = MODE_GAME;
                }
