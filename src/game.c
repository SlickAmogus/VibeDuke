...
void Logo(void)
{
    ...
    setview(0,0,xdim-1,ydim-1);
#ifdef _XBOX
    xbox_log("DUKE3D: Logo clearallviews\n");
#endif
    clearallviews(0L);
#ifdef _XBOX
    xbox_log("DUKE3D: Logo palto\n");
#endif
    IFISSOFTMODE palto(0,0,0,63);
#ifdef _XBOX
    xbox_log("DUKE3D: Logo flushperms\n");
#endif
    flushperms();
#ifdef _XBOX
    xbox_log("DUKE3D: Logo nextpage\n");
#endif
    nextpage();
#ifdef _XBOX
    xbox_log("DUKE3D: Logo stopmusic\n");
#endif
    stopmusic();
    FX_StopAllSounds(); // JBF 20031228
    clearsoundlocks();  // JBF 20031228
#ifdef _XBOX
    xbox_log("DUKE3D: Logo playanm block\n");
#endif
...
if (VOLUMEALL) {
    ...
    if(!KB_KeyWaiting() && nomorelogohack == 0)
    {
        getpackets();
#ifdef _XBOX
        xbox_log("DUKE3D: Logo playanm\n");
#endif
        playanm("logo.anm",5);
#ifdef _XBOX
        xbox_log("DUKE3D: Logo playanm done\n");
#endif
        IFISSOFTMODE palto(0,0,0,63);
        KB_FlushKeyboardQueue();
        KB_ClearKeysDown(); // JBF
...
#ifdef _XBOX
    xbox_log("DUKE3D: Logo playmusic\n");
#endif
    playmusic(&env_music_fn[0][0]);
#ifdef _XBOX
    xbox_log("DUKE3D: Logo fadepal1\n");
#endif
    if (!NAM) {
        fadepal(0,0,0, 0,64,7);
        ...
        KB_FlushKeyboardQueue();
#ifdef _XBOX
        xbox_log("DUKE3D: Logo drealms loop\n");
#endif
        do {
            handleevents();
            getpackets();
...
#ifdef _XBOX
            if (BUTTON(gamefunc_Show_Menu)) { CONTROL_ClearButton(gamefunc_Show_Menu); break; }
#endif
...
#ifdef _XBOX
    xbox_log("DUKE3D: initengine\n");
#endif
    if (initengine()) {
#ifdef _XBOX
       xbox_log("XBOX: initengine FAILED: %s\n", engineerrstr ? engineerrstr : "?");
       for (volatile int _d=0; _d<500000000; _d++); /* pause so msg is readable */
       exit(1);
#else
...
#ifdef _XBOX
    xbox_log("DUKE3D: initengine done\n");
#endif

#ifdef _XBOX
    xbox_log("DUKE3D: compilecons\n");
#endif
    compilecons();
#ifdef _XBOX
    xbox_log("DUKE3D: CONTROL_Startup\n");
#endif
...
#ifdef _XBOX
    xbox_log("DUKE3D: SetupGameButtons\n");
#endif
    SetupGameButtons();
    CONFIG_SetupMouse();
...
#ifdef _XBOX
    xbox_log("DUKE3D: inittimer\n");
#endif
    inittimer(TICRATE, NULL);

#ifdef _XBOX
    xbox_log("DUKE3D: loadpics\n");
#endif
...
#ifdef _XBOX
    xbox_log("DUKE3D: genspriteremaps\n");
#endif
    buildprintf("Loading palette/lookups.\n");
    genspriteremaps();

#ifdef _XBOX
    xbox_log("DUKE3D: readsavenames\n");
#endif
    readsavenames();
...
#ifdef _XBOX
    xbox_log("DUKE3D: app_main start\n");
#endif
...
#ifdef _XBOX
    // Add search paths for game data. Try both D:\ (disc/FTP root) and
    // E:\jfduke3d (HDD install). addsearchpath uses Bstat which may fail
    // on bare drive roots, so try subdirectory forms too.
    {
        static const char * const xbox_paths[] = {
            "D:\\",
            "D:\\music",
            "D:\\jfduke3d",
            "E:\\Apps\\jfduke3d",
            "E:\\Apps\\jfduke3d\\music",
            "E:\\jfduke3d",
            "E:\\jfduke3d\\music",
            "E:\\",
            NULL
        ...
        for (int i = 0; xbox_paths[i]; i++) {
            int ret = addsearchpath(xbox_paths[i]);
            xbox_log("XBOX: addsearchpath(%s)=%d\n", xbox_paths[i], ret);
        }
    }
#endif
...
#ifndef _XBOX
    // Skip on Xbox: startwin_scan_gamedata() CRCs the entire 44MB GRP before SDL
    // is even initialized, causing 30+ seconds of garbage framebuffer. On Xbox
    // we always use the default duke3dgrp ("duke3d.grp") found via addsearchpath.
...
#ifdef _XBOX
    xbox_log("DUKE3D: GRP=%s\n", duke3dgrp);
#endif
    buildprintf("GRP file: %s\n", duke3dgrp);
    {
        int grpret = initgroupfile(duke3dgrp);
#ifdef _XBOX
        xbox_log("XBOX: initgroupfile=%d\n", grpret);
#endif
    }
...
#ifdef _XBOX
    xbox_log("DUKE3D: Startup (SDL init)\n");
#endif
    Startup();
#ifdef _XBOX
    xbox_log("DUKE3D: Startup done\n");
#endif
...
#ifdef _XBOX
    xbox_log("DUKE3D: setgamemode done\n");
#endif
    {
         ...
         * SDL_CreateWindow/Renderer may reconfigure the NV2A/MCPX in a way
         * that disrupts the Connexant CX25871 AV encoder's audio routing.
         * Calling XVideoSetMode again ensures AvSetDisplayMode re-programs
         * the encoder for both video AND audio output. */
        {
            VIDEO_MODE vm = XVideoGetMode();
            xbox_log("DUKE3D: Re-asserting XVideoSetMode(%d,%d) before audio init\n",
                vm.width, vm.height);
            XVideoSetMode(vm.width > 0 ? vm.width : 640,
        ...
        SoundStartup();
#ifdef _XBOX
        /* Play a test sound immediately after SoundStartup to verify the FX pipeline. */
        sound(EXITMENUSOUND);
        xbox_log("DUKE3D: test sound triggered (EXITMENUSOUND=%d)\n", (int)EXITMENUSOUND);
        xbox_log("DUKE3D: MusicStartup\n");
#endif
        buildprintf("Checking music inits.\n");
        MusicStartup();
#ifdef _XBOX
        xbox_log("DUKE3D: loadtmb\n");
#endif
        loadtmb();
#ifdef _XBOX
        xbox_log("DUKE3D: after loadtmb\n");
#endif
    }
...
#ifdef _XBOX
    xbox_log("DUKE3D: setbrightness\n");
#endif
    setbrightness(ud.brightness>>2,&ps[myconnectindex].palette[0],0);

    //ESCESCAPE;
#ifdef _XBOX
    xbox_log("DUKE3D: FX_StopAllSounds\n");
#endif
    FX_StopAllSounds();
    clearsoundlocks();
...
//    getpackets();
    ...
    MAIN_LOOP_RESTART:
#ifdef _XBOX
    xbox_log("DUKE3D: MAIN_LOOP_RESTART\n");
#endif
    ...
    {
#ifdef _XBOX
        xbox_log("DUKE3D: Logo\n");
#endif
        Logo();
#ifdef _XBOX
        xbox_log("DUKE3D: after Logo\n");
#endif
    }
...
#ifdef _XBOX
        xbox_log("DUKE3D: playback fadepal\n");
#endif
        fadepal(0,0,0, 0,63,7);
#ifdef _XBOX
        xbox_log("DUKE3D: playback setgamepalette\n");
#endif
        setgamepalette(&ps[myconnectindex], palette, 1);    // JBF 20040308
#ifdef _XBOX
        xbox_log("DUKE3D: playback drawbackground\n");
#endif
        drawbackground();
#ifdef _XBOX
        xbox_log("DUKE3D: playback menus\n");
#endif
        menus();
        //ps[myconnectindex].palette = palette;
        nextpage();
        ...
        if(foundemo == 0)
        {
...
#ifdef _XBOX
            {
                ...
                if (!once) {
                    ...
                    sprintf(xdbuf, "DUKE3D: loop xdim=%d ydim=%d gm=%d\n", xdim, ydim, ps[myconnectindex].gm);
                    xbox_log(xdbuf);
                    once = 1;
                }
            }
...
#ifdef _XBOX
        // On Xbox with no demo, always keep menu visible (no keyboard to reopen it,
        // and CONTROL_GetInput is not called here so BUTTON() state is stale)
        if (foundemo == 0 && !(ps[myconnectindex].gm&MODE_MENU)) {
            ps[myconnectindex].gm |= MODE_MENU;
            cmenu(0);
        }
#endif
